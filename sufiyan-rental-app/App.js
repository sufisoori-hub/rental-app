...
export default function App() {
  ...
  // Schedule notification for due rent
  ...
  // Calculate totals
  const totalCollected = carts
    .filter((c) => c.rentStatus === "Paid")
    .reduce((sum, c) => sum + parseFloat(c.monthlyRent || 0), 0);

  const totalPending = carts
    .filter((c) => c.rentStatus !== "Paid")
    .reduce((sum, c) => sum + parseFloat(c.monthlyRent || 0), 0);
  ...
  const handlePreviewFile = (file) => {
    if (file?.uri) Linking.openURL(file.uri);
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TextInput
        style={styles.input}
        placeholder="Search Cart ID / Renter / Status"
        value={searchTerm}
      ...
      {filteredCarts.map(cart => (
        <View key={cart.cartId} style={styles.card}>
          ...
          {cart.locationLink && (
            <TouchableOpacity onPress={() => Linking.openURL(cart.locationLink)}>
              <Text style={{color:'blue'}}>View Location</Text>
            </TouchableOpacity>
          )}
